...
pub fn build_db_model(
    ...
    let key_field = info.get_key_field();
...
fn build_db_document_impl(
    ...
    let collection_type_name = if let Some(collection_type) = &options.collection_type {
        format_ident!("{}", collection_type)
    } else {
        format_ident!("CollectionKind")
    };
...
fn build_sync_impl(
    ...
    let config_collection_key_method =
        if let Some(sync_collection_key_method) = &options.sync_collection_key_method {
            sync_collection_key_method.clone()
        } else {
            format_ident!("collection_mutex_key")
        };
