...
use crate::types::filters::{APIFilteringStatsConfig, DBDataType};
    ...
    pub fn validate(&self, context: &T::Context) -> Result<(), &'static str> {
        match self {
            APIFilterField::Field(v) => {
                if v.is_valid_for_filtering(context) {
                    Ok(())
                } else {
            ...
            APIFilterField::Function(v) => v.validate(context),
        }
    }
...
impl<T: PaginatedDocumentField> APIFilterFieldFunction<T> {
    ...
    pub fn validate(&self, context: &T::Context) -> Result<(), &'static str> {
        self.name.validate(&self.args, context)
    }
...
//         let field =
//             APIFilterField::<ChallengeAPIDocumentField>::Field(ChallengeAPIDocumentField::Name);
//         let mut query = String::new();
//         let mut aql = AqlBuilder::new_for_in_collection(AQL_DOCUMENT_ID, "Collection");
//         field.build_aql(&mut query, &mut aql).unwrap();
//
